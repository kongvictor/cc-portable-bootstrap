...
Version 1.2 also removes the exact legacy local section headed `# CodexDev / CodexDevFast 触发词（用户自定义，非 managed block）` when all known signatures match. A same-heading section with different content fails closed for manual review rather than deleting user-authored rules.
...
Version 1.2 replaces the tier-only shortcuts (`claudexhigh`, `claudexxhigh`, `claudexmax`, `claudexultra`, and their Fast variants) with model-aware names such as `claudexsolxhigh`, `claudexlunamaxfast`, and `claudexterraultrafast`. Setup removes an obsolete shortcut only when its contents exactly match a wrapper generated by an older bootstrap release. Modified files, symlinks, and non-files are preserved and reported. Removed wrappers are included in the normal setup backup and restored by rollback or `restore`.
