...
- 触发词：`CodexDev`。用户说“使用 CodexDev”或在消息中用 CodexDev 指名当前任务时，仅当前任务启用；说“进入 CodexDev”时，后续任务持续启用，直到用户说“退出 CodexDev”。
...
- 快速变体触发词：`CodexDevFast`，启用/退出语义与 CodexDev 相同。启用后所有规则均与 CodexDev 相同，包括模型、effort、sandbox、`approval-policy` 和 `threadId` 复用；唯一区别是每次调用 `mcp__codex__codex` 时都在 `config` 中额外传 `"service_tier": "fast"`。这是 Codex 原生 fast tier（请求值 `priority`），基础设施优先级提速约 1.5×，推理深度不变；ChatGPT 登录下 credit 约为 Standard 的 2.5×。普通 CodexDev 不传 `service_tier`。
